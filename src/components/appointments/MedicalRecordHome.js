                        ...
                        {medical.length > 0 ? (
                            <View style={{ flexDirection: 'column', gap: 5, width: '95%', paddingVertical: 10 }}>
                                {medical.map((item, index) => (
                                    <View style={{ flexDirection: 'row', width: '100%', paddingHorizontal: 10, paddingVertical: 15, borderRadius: 8, backgroundColor: '#f1f1f1', gap: 5, justifyContent: 'flex-start', position: 'relative' }} key={index}>
                                        ...
                                        <TouchableOpacity style={{ position: 'absolute', right: 5, top: 20 }} onPress={() => setMedical(prev => prev.filter(item1 => item1.medicalName !== item.medicalName))}>
